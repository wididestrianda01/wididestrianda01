...
| --- | --- | --- |
| [systematic-futures-lab](https://github.com/wididestrianda01/systematic-futures-lab) | Systematic futures method-comparison lab on 16 CME roots, 2010–2024Q1: trend, cross-sectional momentum, carry and seasonality against two LightGBM variants, through one accounting engine, one cost model, and a single out-of-sample read across a 0/2/5/10 bps cost ladder. Costs decide the comparison — cross-sectional momentum is the only family positive in every sub-period and in both windows (0.406 in, 1.174 out at 2 bps) — and both ML variants clear the pre-declared decision rule inside the window (DSR 0.9989/0.9999) then fail it out of time, reported as the finding rather than retuned. Deflated Sharpe and purged, embargoed folds price the search; 123 tests. | Python |
...
| [commodity-hedging-workbench](https://github.com/wididestrianda01/commodity-hedging-workbench) | Commodity hedging and structuring workbench extending the Citi MQA Forage simulation onto frozen ICE coffee data: cost-of-carry calibration with a Schwartz–Smith two-factor fit, GARCH(1,1) working volatility, a zero-cost collar that cuts a coffee roaster's 95th-percentile cost peak by 58%, a capital-protected note priced by Black-76 against antithetic Monte Carlo, and historical VaR/ES with a bucketed CVA layer, under SR 26-2-style validation with SHA-256-frozen data gates. | Python |
